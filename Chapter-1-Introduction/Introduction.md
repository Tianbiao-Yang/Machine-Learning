...
* 交叉验证法：先把数据集分成k个大小相似的互斥子集，在通过分层抽样以保证数据分布一致，然后用k-1个子集的并集作为训练集，余下的子集作为测试集，这样可以获得k组训练/测试集合，进行k次训练和测试，返回k个结果的均值。<img src="http:..." width = "100" height = "100" div align=center />![](../Image-Gallery/1-5三折交叉验证.png)
	<div align=left>* matlab函数是：[indices = crossvalind('Kfold',data,k)](../Chapter-1-Introduction/matlab_codes/cross_validation.m);k若为1则是留一法（LOO）
	* 
